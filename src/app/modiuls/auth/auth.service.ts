...
const loginUserFeomDb=async(payloads:TLoginUser)=>{
    ...
    if(!user){
        throw new AppError(httpStatus.BAD_REQUEST,' This email is not Exisit - Please create Account')
    }
    ...
    if(!isPasswordMatch){
        throw new AppError(httpStatus.BAD_REQUEST,'Incorrecr password - Please Try again')
    }
    if(user.isDeleted){
